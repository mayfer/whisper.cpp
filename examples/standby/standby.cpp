...
int run_loaded_whisper(struct whisper_context * ctx, whisper_params &params, char ** argv) {
        ...
        // WAV input
        {
            ...
            if (fname_inp == "-") {
                ...
                if (drwav_init_memory(&wav, wav_data.data(), wav_data.size(), nullptr) == false) {
                    ...
                    fprintf(stderr, "--- contents of stdin ---\n");
                    fprintf(stderr, "%s\n", wav_data.data());
                    return 4;
                }
    ...
    while(true) {
        ...
        if (result == -1) {
            ...
            while(std::getline(std::cin, text)) {
                // text is filename, add it to params
                if(text != "") {
                    params.fname_inp[0] = text;
                }
                //fprintf(stdout, "/// Got input: %s\n", text.c_str());
                //fprintf(stdout, "/// Running with prompt: %s\n", params.prompt.c_str());
                int idk = run_loaded_whisper(ctx, params, argv);
                //fprintf(stdout, "/// Done\n");
                //}
                std::this_thread::sleep_for(std::chrono::milliseconds(1000));
                std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
                clear_prompt_past(ctx);
